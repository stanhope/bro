...
namespace analyzer { namespace dns_telemetry {
...
class DNS_Telemetry_MsgInfo {
public:
	DNS_Telemetry_MsgInfo(DNS_RawMsgHdr* hdr, int is_query);
	~DNS_Telemetry_MsgInfo();
...
class DNS_Telemetry_Interpreter {
public:
	DNS_Telemetry_Interpreter(analyzer::Analyzer* analyzer);
...
protected:
	...
	int ParseRR_Name(DNS_Telemetry_MsgInfo* msg,
				const u_char*& data, int& len, int rdlength,
				const u_char* msg_start);
	int ParseRR_SOA(DNS_Telemetry_MsgInfo* msg,
				const u_char*& data, int& len, int rdlength,
				const u_char* msg_start);
	int ParseRR_MX(DNS_Telemetry_MsgInfo* msg,
				const u_char*& data, int& len, int rdlength,
				const u_char* msg_start);
	int ParseRR_NBS(DNS_Telemetry_MsgInfo* msg,
				const u_char*& data, int& len, int rdlength,
				const u_char* msg_start);
	int ParseRR_SRV(DNS_Telemetry_MsgInfo* msg,
				const u_char*& data, int& len, int rdlength,
				const u_char* msg_start);
	int ParseRR_EDNS(DNS_Telemetry_MsgInfo* msg,
				const u_char*& data, int& len, int rdlength,
				const u_char* msg_start);
	int ParseRR_A(DNS_Telemetry_MsgInfo* msg,
				const u_char*& data, int& len, int rdlength);
	int ParseRR_AAAA(DNS_Telemetry_MsgInfo* msg,
				const u_char*& data, int& len, int rdlength);
	int ParseRR_WKS(DNS_Telemetry_MsgInfo* msg,
				const u_char*& data, int& len, int rdlength);
	int ParseRR_HINFO(DNS_Telemetry_MsgInfo* msg,
				const u_char*& data, int& len, int rdlength);
	int ParseRR_TXT(DNS_Telemetry_MsgInfo* msg,
				const u_char*& data, int& len, int rdlength,
				const u_char* msg_start);
	int ParseRR_TSIG(DNS_Telemetry_MsgInfo* msg,
				const u_char*& data, int& len, int rdlength,
				const u_char* msg_start);

	void SendReplyOrRejectEvent(DNS_Telemetry_MsgInfo* msg, EventHandlerPtr event,
					const u_char*& data, int& len,
...
class Contents_DNS_Telemetry : public tcp::TCP_SupportAnalyzer {
public:
	Contents_DNS_Telemetry(Connection* c, bool orig, DNS_Telemetry_Interpreter* interp);
	~Contents_DNS_Telemetry();
...
protected:
	...
	DNS_Telemetry_Interpreter* interp;
...
class DNS_Telemetry_Analyzer : public tcp::TCP_ApplicationAnalyzer {
public:
	DNS_Telemetry_Analyzer(Connection* conn);
	~DNS_Telemetry_Analyzer();
	...
	static analyzer::Analyzer* InstantiateAnalyzer(Connection* conn)
		{ return new DNS_Telemetry_Analyzer(conn); }

protected:
	DNS_Telemetry_Interpreter* interp;
	Contents_DNS_Telemetry* contents_dns_orig;
	Contents_DNS_Telemetry* contents_dns_resp;
