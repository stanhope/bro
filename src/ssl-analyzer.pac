	...
	function proc_certificate(rec: SSLRecord, certificates : bytestring[]) : bool
		...
		if ( x509_certificate )
			...
			for ( unsigned int i = 0; i < certificates->size(); ++i )
				...
				StringVal* der_cert_copy = new StringVal(cert.length(), (const char*) cert.data()); // copy for second event. 
				...
				BifEvent::generate_x509_certificate(bro_analyzer(), bro_analyzer()->Conn(),
							${rec.is_orig},
							pX509Cert->Ref(), // ref'd because used in _done event
							i, certificates->size(),
							der_cert); 
				...
				BifEvent::generate_x509_certificate_done(bro_analyzer(), bro_analyzer()->Conn(), ${rec.is_orig},
									pX509Cert, der_cert_copy);
				}
			}
		return true;
		%}
